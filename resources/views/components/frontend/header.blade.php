            ...
            <div class="flex items-center gap-4">
                @auth
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center gap-2 text-amber-900">
                            <span>{{ auth()->user()->name }}</span>
                        </button>
                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2">
                            <a href="{{ route('coming.soon') }}" class="block px-4 py-2 text-amber-900 hover:bg-gray-100">Meu Perfil</a>
                            <a href="{{ route('coming.soon') }}" class="block px-4 py-2 text-amber-900 hover:bg-gray-100">Meus Pedidos</a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="block w-full text-left px-4 py-2 text-amber-900 hover:bg-gray-100">
                                    Sair
                                </button>
                            </form>
                        </div>
                    </div>
                    <a href="{{ route('account.cart.index') }}" class="relative">
                        <svg class="w-6 h-6 text-gray-700" fill="none" stroke="#461901" viewBox="0 0 24 24">
